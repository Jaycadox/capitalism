...
public class TaxTransaction {
	...
	public TransactionResult transact(ITax taxProvider, boolean silent) {
		...
		if ((int) tax.getAmountTaxed() + amount > DatabasePlayer.from(from).getMoneySafe()) {
			if (!silent)
				new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount)).appendCaption("could not be transfered due to").appendVariable("insufficient funds").send(Bukkit.getPlayer(from));
		...
		if (!silent)
			new MessageBuilder("Transaction Tax").appendVariable("$" + NumberFormatter.addCommas(tax.getAmountTaxed())).appendCaption("has been removed from your account following a").appendVariable(tax.getTaxAmount() * 100 + "%").appendCaption("tax").send(Bukkit.getPlayer(from));
		...
		if (res.getType() != TransactionResult.TransactionResultType.SUCCESS) {
			if (!silent)
				new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount)).appendCaption("could not be transferred to").appendVariable(Bukkit.getOfflinePlayer(to).getName()).appendCaption("due to").appendVariable(res.getErrorReason()).send(Bukkit.getPlayer(from));
