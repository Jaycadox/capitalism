	...
	public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
		...
		InventoryHelper inventoryHelper = new InventoryHelper(p.getName() + "'s profile", 3, (helper, submenu) -> {
			...
			if (submenu.equals("claims")) {
				...
				if (scroll[0] == null)
					scroll[0] = new InventoryScroll(0, 6, helper, itemRunnables, submenu, p.getUniqueId());
			...
			if (submenu.equals("offers")) {
				List<InventoryScroll.ItemRunnable> itemRunnables = databasePlayer.getJsonPlayer().getClaimOffers().stream().map(x -> {
					...
					ItemStack it = new ItemStack(count.get() % 2 == 0 ? Material.MAP : Material.FILLED_MAP);
					ItemMeta meta = it.getItemMeta();
					meta.setDisplayName(ChatColor.GREEN + "$" + NumberFormatter.addCommas(x.price) + " " + ChatColor.YELLOW + c.getName() + ChatColor.GRAY + " (" + c.getArea() + " blocks)");
					meta.setLore(List.of("&7Location: &e" + area, "&7Market value: &e$" + NumberFormatter.addCommas(c.getEstWorth()), "&7Owner: &e" + Bukkit.getOfflinePlayer(UUID.fromString(c.owner)).getName(), "", "&7Left-click to: &a&lBUY", "&7Right-click to: &c&lREJECT").stream().map(msg -> ChatColor.translateAlternateColorCodes('&', msg)).collect(Collectors.toList()));
					it.setItemMeta(meta);
					Runnable r = () -> {
						if (InventoryHelperEvent.CLICK_TYPES.get(p.getUniqueId())) {
							...
							if (result.getType() != TransactionResult.TransactionResultType.SUCCESS) {
								new MessageBuilder("Land Purchase").appendCaption("Transaction failed due to:").appendVariable(result.getErrorReason()).send(p);
								return;
							}
							...
							DatabasePlayer.from(other.getUniqueId()).getJsonPlayer().queueMessage(new MessageBuilder("Land").appendCaption("You no longer own the land at").appendVariable(area + ".").appendCaption("This is because ownership has been purchased by").appendVariable(p.getName()).appendCaption("for").appendVariable("$" + NumberFormatter.addCommas(x.price)).make());
							
							new MessageBuilder("Land").appendCaption("You now own the land at").appendVariable(area + ".").appendCaption("This action has costed you").appendVariable("$" + NumberFormatter.addCommas(transaction.getTotalAmount(TaxedTransaction.INSTANCE))).send(p);
							InventoryHelper.close(p);
						} else {
							DatabasePlayer.from(UUID.fromString(DatabasePlayer.getClaimFromClaimOffer(x).owner)).deleteAllOffersForClaim(DatabasePlayer.getClaimFromClaimOffer(x));
							DatabasePlayer.from(UUID.fromString(DatabasePlayer.getClaimFromClaimOffer(x).owner)).getJsonPlayer().save();
							new MessageBuilder("Land").appendCaption("Offer has been rejected").send(p);
							DatabasePlayer.from(UUID.fromString(DatabasePlayer.getClaimFromClaimOffer(x).owner)).getJsonPlayer().queueMessage(new MessageBuilder("Land").appendCaption("Your land claim offer to").appendVariable(p.getName()).appendCaption("has been rejected").make());
						}
				...
				if (offersScroll[0] == null)
					offersScroll[0] = new InventoryScroll(0, 6, helper, itemRunnables, submenu, p.getUniqueId());
			...
			if (submenu.equals("bank")) {
				...
				}, List.of("&7Transaction tax: &e" + (int) ( 100 * TaxedTransaction.INSTANCE.applyTax((int) databasePlayer.getMoneySafe()).getTaxAmount() ) + "%", "&7Death tax: &e" + (int) ( 100 * TaxedDeath.INSTANCE.applyTax((int) databasePlayer.getMoneySafe()).getTaxAmount() ) + "%"));
			}
		});
