		...
		InventoryHelper inventoryHelper = new InventoryHelper(p.getName() + "'s profile", 3, (helper, submenu) -> {
			...
			if(submenu.equals("claims")) {
				...
				if(scroll[0] == null)
					...
					scroll[0].setView(itemRunnables);
				scroll[0].render();
			}
			if(submenu.equals("offers")) {
				List<InventoryScroll.ItemRunnable> itemRunnables = databasePlayer.getJsonPlayer().getData().claimOffers.stream().map(
						x -> {
							...
							meta.setLore(List.of(
									"&7Location: &e" + area,
									"&7Market value: &e$" + NumberFormatter.addCommas(c.getEstWorth()),
									"&7Owner: &e" + Bukkit.getOfflinePlayer(UUID.fromString(c.owner)).getName(),
									"",
									"&7Left-click to: &a&lBUY",
								...
								if(InventoryHelperEvent.CLICK_TYPES.get(p.getUniqueId())) {
									...
									DatabasePlayer.from(other.getUniqueId()).getJsonPlayer().queueMessage(
											new MessageBuilder("Land").appendCaption("You no longer own the land at")
													.appendVariable(area + ".")
													.appendCaption("This is because ownership has been purchased by")
				...
				if(offersScroll[0] == null)
					...
					offersScroll[0].setView(itemRunnables);
