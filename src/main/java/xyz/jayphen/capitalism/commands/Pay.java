...
public class Pay implements CommandExecutor {
	...
	public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
		...
		if ((int) tax.getAmountTaxed() + amount > DatabasePlayer.from(p).getMoneySafe()) {
			new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount)).appendCaption("could not be transfered due to").appendVariable("insufficient funds").send(commandSender);
			return true;
		}
		...
		if (tax_res.getType() != TransactionResult.TransactionResultType.SUCCESS) {
			new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount + (int) tax.getAmountTaxed())).appendCaption("(after tax) could not be sent due to").appendVariable(tax_res.getErrorReason()).send(commandSender);
			return true;
		}
		DatabasePlayer.from(p).getJsonPlayer().getData().stats.amountTaxed += tax.getAmountTaxed();
		DatabasePlayer.from(p).getJsonPlayer().save();
		new MessageBuilder("Transaction Tax").appendVariable("$" + NumberFormatter.addCommas(tax.getAmountTaxed())).appendCaption("has been removed from your account following a").appendVariable(tax.getTaxAmount() * 100 + "%").appendCaption("tax").send(commandSender);
		...
		if (res.getType() != TransactionResult.TransactionResultType.SUCCESS) {
			new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount)).appendCaption("could not be transferred to").appendVariable(otherPlayer.getName()).appendCaption("due to").appendVariable(res.getErrorReason()).send(commandSender);
			return true;
		}
		new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount)).append(Token.TokenType.BRACKET, res.getHash()).appendCaption("has been transferred to").appendVariable(otherPlayer.getName() + ".").appendCaption("You now have").appendVariable("$" + NumberFormatter.addCommas(DatabasePlayer.from(p).getMoneySafe())).send(commandSender);
		if (otherPlayer.isOnline() && otherPlayer.getPlayer() != null) {
			new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount)).append(Token.TokenType.BRACKET, res.getHash()).appendCaption("has been transferred to your balance. You how have").appendVariable("$" + NumberFormatter.addCommas(DatabasePlayer.from(otherPlayer).getMoneySafe())).send(otherPlayer.getPlayer());
		}
