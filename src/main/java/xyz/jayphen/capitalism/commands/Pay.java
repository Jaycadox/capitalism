...
public class Pay implements CommandExecutor {
	...
	public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
		...
		if (!otherPlayer.hasPlayedBefore()) {
			new MessageBuilder("Economy").appendCaption("The player").appendVariable(args[0]).appendCaption("could not be found")
					.send(commandSender);
			return true;
		}
		...
		if (res.getType() != TransactionResult.TransactionResultType.SUCCESS) {
			new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount))
					.appendCaption("could not be transferred to").appendVariable(otherPlayer.getName()).appendCaption("due to")
					.appendVariable(res.getErrorReason()).send(commandSender);
			return true;
		}
		new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount))
				.append(Token.TokenType.BRACKET, res.getHash()).appendCaption("has been transferred to")
				.appendVariable(otherPlayer.getName() + ".").appendCaption("You now have")
		...
		if (otherPlayer.isOnline() && otherPlayer.getPlayer() != null) {
			new MessageBuilder("Transaction").appendVariable("$" + NumberFormatter.addCommas(amount))
					.append(Token.TokenType.BRACKET, res.getHash()).appendCaption("has been transferred to your balance. You how have")
