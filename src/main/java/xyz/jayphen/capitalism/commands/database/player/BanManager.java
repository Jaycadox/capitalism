...
public class BanManager {
	public static void applyBan(OfflinePlayer p, String reason, long length) {
		...
		var message = Component.text("Applied account ban to ", NamedTextColor.GREEN).append(Component.text(Objects.requireNonNull(p.getName()), NamedTextColor.YELLOW)).append(Component.text(". Reason given: ", NamedTextColor.GREEN)).append(Component.text(reason, NamedTextColor.YELLOW)).append(Component.text(". Time until unban: ", NamedTextColor.GREEN)).append(Component.text(TimeHelper.timeToString(length), NamedTextColor.YELLOW));
	...
	public static Component getBanMessage(DatabasePlayer dbp, long timeLeft) {
		...
		return MiniMessage.miniMessage().deserialize("[CAPITALISM SMP]<newline><color:red><bold>INFRACTION NOTICE</bold></color><newline><newline><yellow>\"" + getFormalBanReason(dbp) + "\"<newline><newline><reset>Expires in <yellow>" + formattedTimeLeft + "<newline><newline>" + ( dbp.getJsonPlayer().getBanReason().contains("[wipe]") ? "<grey>In addition, your player data has been reset<newline><newline><reset>" : "" ) + ( dbp.getJsonPlayer().getBanReason().equals("[ac]") ? "<grey>This action was performed automatically, with no human involvement<newline><newline><reset>" : "" ) + "<gray>If you wish to appeal this infraction, please message <yellow>jayphen#6666<gray> on Discord");
	}
}
