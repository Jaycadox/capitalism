...
public class Land implements CommandExecutor, TabCompleter {
	...
	private void asyncCommandHandler(CommandSender commandSender, String[] args, Claim oClaim) {
			...
			InventoryHelper inventoryHelper = new InventoryHelper("Landlord > " + ClaimManager.getDatabaseClaim(c).getName(), 1, (inv, ctx) -> {
				...
				if (ctx.equals("destroy")) {
					...
					inv.setItem(0, 0, ChatColor.GREEN + "" + ChatColor.BOLD + "CONFIRM", Material.GREEN_WOOL, () -> {
						...
						new MessageBuilder("Land").appendCaption("Land claim has been destroyed. Please allow up to").appendVariable("8 seconds").appendCaption("for changes to take effect").send(p);
						InventoryHelper.close(p);
					});
					inv.addMargin(0, 1);
				}
				if (ctx.equals("trusted")) {
					...
					var list = ClaimManager.getDatabaseClaim(c).getTrusted().stream().map(x -> {
						...
						return new InventoryScroll.ItemRunnable(InventoryHelper.getHead(player, ChatColor.YELLOW + player.getName(), List.of("&7Click to: &c&lUN-TRUST PLAYER")), () -> {
				...
				if (ctx.equals("selltransfer")) {
					...
					inv.setItem(0, 2, ChatColor.YELLOW + "Sell Claim", Material.GOLD_INGOT, () -> {
						...
						ChatInput.createQuery("amount of money to sell for", response -> {
							...
							ChatInput.createQuery("player name to sell land to", name -> {
								...
								DatabasePlayer.from(tPlayer.getUniqueId()).getJsonPlayer().getClaimOffers().add(new ClaimOffer(new ClaimLocation(c.location.startX, c.location.startZ, c.location.endX, c.location.endZ, c.location.world), finalAmount));
								DatabasePlayer.from(tPlayer.getUniqueId()).getJsonPlayer().save();
								new MessageBuilder("Land").appendCaption("Created offer for").appendVariable("$" + NumberFormatter.addCommas(finalAmount)).appendCaption("to").appendVariable(tPlayer.getName()).send(p);
								DatabasePlayer.from(tPlayer.getUniqueId()).getJsonPlayer().queueMessage(new MessageBuilder("Land").appendVariable(p.getName()).appendCaption("is offering to sell you a land claim, type").appendVariable("/profile").appendCaption("for more info").make());
							}, p);
						}, p);
						inv.push("sell", 1);
					});
					inv.setItem(0, 4, ChatColor.RED + "Transfer Claim", Material.REDSTONE_BLOCK, () -> {
						...
						ChatInput.createQuery("player name to transfer claim to", response -> {
							...
							new MessageBuilder("Land").appendCaption("You no longer own the land at").appendVariable(area + ".").appendCaption("This is because ownership has been transferred to").appendVariable(tPlayer.getName()).send(p);
							DatabasePlayer.from(tPlayer.getUniqueId()).getJsonPlayer().queueMessage(new MessageBuilder("Land").appendCaption("You now own the land at").appendVariable(area + ".").appendCaption("This was free as it has been transferred to you by").appendVariable(p.getName()).make());
				...
				if (ctx.equals("info")) {
					inv.setMargin(1, 0);
					inv.setItem(0, 0, ChatColor.YELLOW + "Claim Area", Material.GRASS_BLOCK, () -> {}, Stream.of("&7This claim's area is: &e" + c.getArea() + " blocks", "&7Location: &e" + c.getMidpointX() + ", " + c.getMidpointZ(), "&7Type: &e" + RegionManager.getRegion(new Location(Bukkit.getWorld(c.location.world), c.location.startX, 0, c.location.startZ)).toString().toLowerCase()).map(x -> ChatColor.translateAlternateColorCodes('&', x)).collect(Collectors.toList()));
					
					inv.setItem(0, 5, ChatColor.RED + "Destroy Claim", Material.TNT, () -> {
						inv.push("destroy", 1);
					}, Stream.of("&7This action is &c&lIRREVERSIBLE!", "&4&lYou will NOT receive any money as a result of this").map(x -> ChatColor.translateAlternateColorCodes('&', x)).collect(Collectors.toList()));
					if (DatabasePlayer.from(p).getClaimOffersFromClaim(c).isEmpty()) {
						...
						}, List.of("&7Offer to player: &e" + Bukkit.getOfflinePlayer(DatabasePlayer.getRecipientOfClaimOffer(offer)).getName(), "&7Offered for: &e$" + NumberFormatter.addCommas(offer.price)));
					}
					
					
					inv.setItem(0, 1, ChatColor.YELLOW + "Claim Value", Material.GREEN_DYE, () -> {}, Stream.of("&7This claim's estimated value is: &a$" + NumberFormatter.addCommas(c.getEstWorth())).map(x -> ChatColor.translateAlternateColorCodes('&', x)).collect(Collectors.toList()));
					inv.setItem(0, 2, InventoryHelper.getHead(Bukkit.getOfflinePlayer(UUID.fromString(c.owner)), "&eClaim Owner", List.of("&7Username: &e" + Bukkit.getOfflinePlayer(UUID.fromString(c.owner)).getName())), () -> {});
				...
				if (ctx.equals("settings")) {
					...
					for (Map.Entry<String, BooleanRunnable> entry : prefs.entrySet()) {
						...
						}, Stream.of("&7Current value: " + ( value ? "&a&lENABLED" : "&c&lDISABLED" ), "", "&e" + getDescription(entry.getKey())).map(x -> ChatColor.translateAlternateColorCodes('&', x)).collect(Collectors.toList()));
		...
		if (args.length >= 1 && args[0].equals("settings")) {
			...
			boolean preferenceToggle = args[2].equalsIgnoreCase("true") || args[2].equalsIgnoreCase("yes") || args[2].equalsIgnoreCase("enable") || args[2].equalsIgnoreCase("on");
			...
			new MessageBuilder("Land").appendVariable(preferenceName).appendCaption("has been").appendVariable(preferenceToggle ? "enabled" : "disabled").send(p);
			DatabasePlayer.from(p).getJsonPlayer().save();
		}
		if (args.length >= 1 && args[0].equals("trusted")) {
			...
			if (args.length == 2 && args[1].equals("list")) {
				if (c.getTrusted().isEmpty()) {
					...
					new MessageBuilder("Land").appendCaption("These players have general trust permissions on this claim:").appendList(ClaimManager.getDatabaseClaim(c).getTrusted().stream().map(x -> Bukkit.getOfflinePlayer(UUID.fromString(x)).getName()).collect(Collectors.toList())).send(p);
					return;
				}
			}
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 3 && args[0].equals("trusted") && args[1].equals("remove")) {
			try {
				return ClaimManager.getCachedClaim(( (Player) sender ).getLocation()).orElse(null).getTrusted().stream().map(x -> Bukkit.getOfflinePlayer(UUID.fromString(x)).getName()).collect(Collectors.toList());
			} catch (Exception ignored) {
			}
			return List.of("");
		}
