...
public class Land implements CommandExecutor, TabCompleter {
	...
	private void asyncCommandHandler(CommandSender commandSender, String[] args, Claim oClaim) {
		...
		if (args.length == 0) {
			InventoryHelper inventoryHelper = new InventoryHelper("Landlord > " + ClaimManager.getDatabaseClaim(c).getName(), 1,
			                                                      (inv, ctx) -> {
				                                                      if (ctx.equals("root")) {
					                                                      inv.setMargin(0, 0);
					                                                      inv.addMargin(0, 0);
					                                                      ...
					                                                      inv.addMargin(0, 7);
					                                                      inv.setItem(0, 8,
					                                                                  InventoryHelper.getHead(p, "&aView profile", null),
					                                                                  () -> {
						                                                                  InventoryHelper.close(p);
						                                                                  p.performCommand("profile");
					                                                                  }
					                                                      );
					                                                      inv.setItem(0, 1 + 3, ChatColor.YELLOW + "Settings",
					                                                                  Material.LODESTONE, () -> {
								                                                      inv.push("settings", 1);
							                                                      }
					                                                      );
					                                                      inv.setItem(0, 0 + 3, ChatColor.YELLOW + "Claim Information",
					                                                                  Material.PAPER, () -> {
								                                                      inv.push("info", 1);
							                                                      }
					                                                      );
					                                                      inv.setItem(0, 2 + 3, ChatColor.YELLOW + "Trusted Players",
				                                                      ...
				                                                      if (ctx.equals("destroy")) {
					                                                      ...
					                                                      inv.setItem(0, 0,
								                                                      ...
								                                                      new MessageBuilder("Land").appendCaption(
												                                                      "Land claim has been destroyed. Please allow up to")
				                                                      ...
				                                                      if (ctx.equals("trusted")) {
					                                                      inv.setMargin(1, 0);
					                                                      var list = ClaimManager.getDatabaseClaim(c).getTrusted().stream()
							                                                      .map(x -> {
								                                                      OfflinePlayer player = Bukkit.getOfflinePlayer(
										                                                      UUID.fromString(x));
								                                                      return new InventoryScroll.ItemRunnable(
										                                                      InventoryHelper.getHead(player,
										                                                                              ChatColor.YELLOW +
										                                                                              player.getName(),
										                                                                              List.of("&7Click to: &c&lUN-TRUST PLAYER")
										                                                      ), () -> {
									                                                      DatabasePlayer.from(p).getJsonPlayer().getClaim(c)
											                                                      .getTrusted().removeIf(pl -> x.equals(
													                                                      player.getUniqueId().toString()));
									                                                      DatabasePlayer.from(p).getJsonPlayer().save();
									                                                      new MessageBuilder("Land").appendVariable(
													                                                      player.getName()).appendCaption(
													                                                      "has been removed from the trust list")
											                                                      .send(p);
								                                                      });
							                                                      }).collect(Collectors.toList());
					                                                      if (scroll[0] == null) {
						                                                      scroll[0] = new InventoryScroll(0, 4, inv, list, ctx,
						                                                                                      p.getUniqueId()
						                                                      );
					                                                      } else {
					                                                      ...
					                                                      scroll[0].render();
					                                                      inv.setItem(0, 5, ChatColor.GREEN + "" + ChatColor.BOLD +
					                                                                        "ADD TRUSTED USER", Material.GREEN_WOOL, () -> {
						                                                      InventoryHelper.close(p);
						                                                      ChatInput.createQuery("player name to be trusted",
						                                                                            response -> {
							                                                                            ...
							                                                                            if (DatabasePlayer.from(p)
									                                                                            .getJsonPlayer().getClaim(c)
									                                                                            .getTrusted().contains(
							                                                                            ...
							                                                                            new MessageBuilder(
									                                                                            ...
									                                                                            .appendCaption(
											                                                                            "has been added to the trust list")
									                                                                            .send(p);
							                                                                            inv.show(p, "trusted");
						                                                                            }, p
						                                                      );
				                                                      ...
				                                                      if (ctx.equals("selltransfer")) {
					                                                      inv.setMargin(1, 0);
					                                                      inv.setItem(0, 2, ChatColor.YELLOW + "Sell Claim",
					                                                                  Material.GOLD_INGOT, () -> {
								                                                      InventoryHelper.close(p);
								                                                      ChatInput.createQuery("amount of money to sell for",
								                                                                            response -> {
									                                                                            InventoryHelper.close(p);
									                                                                            int amount = 0;
									                                                                            try {
									                                                                            ...
									                                                                            if (1 > amount) {
										                                                                            new MessageBuilder(
												                                                                            "Land").appendCaption(
														                                                                            "Cannot sell for a free or negative price")
												                                                                            .send(p);
										                                                                            return;
									                                                                            }
									                                                                            int finalAmount = amount;
									                                                                            ChatInput.createQuery(
											                                                                            "player name to sell land to",
											                                                                            name -> {
														                                                                            ...
														                                                                            .queueMessage(
																                                                                            new MessageBuilder(
																		                                                                            "Land").appendVariable(
																				                                                                            p.getName())
																		                                                                            .appendCaption(
																				                                                                            "is offering to sell you a land claim, type")
																		                                                                            .appendVariable(
																				                                                                            "/profile")
																		                                                                            .appendCaption(
																				                                                                            "for more info")
					                                                      ...
					                                                      inv.setItem(0, 4, ChatColor.RED + "Transfer Claim",
					                                                                  Material.REDSTONE_BLOCK, () -> {
								                                                      InventoryHelper.close(p);
								                                                      ChatInput.createQuery(
										                                                      "player name to transfer claim to",
										                                                      response -> {
											                                                      OfflinePlayer tPlayer
													                                                      = Bukkit.getOfflinePlayer(
													                                                      response);
											                                                      if (!tPlayer.hasPlayedBefore()) {
												                                                      new MessageBuilder(
											                                                      ...
											                                                      String area = "(" + c.location.startX +
											                                                                    ", " + c.location.startZ +
											                                                                    " -> " + c.location.endX +
											                                                                    ", " + c.location.endZ +
											                                                                    ")";
											                                                      ClaimManager.getDatabaseClaim(c)
													                                                      .transfer(tPlayer.getUniqueId());
											                                                      new MessageBuilder("Land").appendCaption(
															                                                      "You no longer own the land at")
													                                                      .appendVariable(area + ".")
													                                                      .appendCaption(
															                                                      "This is because ownership has been transferred to")
													                                                      .appendVariable(tPlayer.getName())
													                                                      .send(p);
											                                                      DatabasePlayer.from(tPlayer.getUniqueId())
													                                                      .getJsonPlayer().queueMessage(
															                                                      new MessageBuilder(
																	                                                      "Land").appendCaption(
																			                                                      "You now own the land at")
																	                                                      .appendVariable(
																			                                                      area + ".")
																	                                                      .appendCaption(
																			                                                      "This was free as it has been transferred to you by")
																	                                                      .appendVariable(
																			                                                      p.getName())
																	                                                      .make());
				                                                      ...
				                                                      if (ctx.equals("info")) {
					                                                      inv.setMargin(1, 0);
					                                                      inv.setItem(0, 0, ChatColor.YELLOW + "Claim Area",
					                                                                  Material.GRASS_BLOCK, () -> {}, Stream.of(
											                                                      "&7This claim's area is: &e" + c.getArea() +
											                                                      " blocks",
											                                                      "&7Location: &e" + c.getMidpointX() + ", " +
											                                                      c.getMidpointZ(), "&7Type: &e" +
					                                                      ...
					                                                      inv.setItem(0, 5, ChatColor.RED + "Destroy Claim", Material.TNT,
					                                                                  () -> {
						                                                                  inv.push("destroy", 1);
					                                                                  }, Stream.of("&7This action is &c&lIRREVERSIBLE!",
					                                                                               "&4&lYou will NOT receive any money as a result of this"
									                                                      ).map(x -> ChatColor.translateAlternateColorCodes('&', x))
							                                                                  .collect(Collectors.toList())
					                                                      );
					                                                      if (DatabasePlayer.from(p).getClaimOffersFromClaim(c).isEmpty()) {
						                                                      inv.setItem(0, 4, ChatColor.YELLOW + "Sell/Transfer Claim",
						                                                                  Material.ENDER_PEARL, () -> {
									                                                      inv.push("selltransfer", 1);
								                                                      }
						                                                      );
					                                                      } else {
						                                                      ClaimOffer offer = DatabasePlayer.from(p)
								                                                      .getClaimOffersFromClaim(c).get(0);
						                                                      inv.setItem(0, 4, ChatColor.RED + "Cancel ongoing offer",
						                                                                  Material.ENDER_EYE, () -> {
									                                                      DatabasePlayer.from(p).deleteAllOffersForClaim(c);
								                                                      }, List.of("&7Offer to player: &e" +
						                                                                         Bukkit.getOfflinePlayer(
								                                                                         DatabasePlayer.getRecipientOfClaimOffer(
										                                                                         offer)).getName(),
						                                                                         "&7Offered for: &e$" +
						                                                                         NumberFormatter.addCommas(offer.price)
								                                                      )
						                                                      );
					                                                      }
					
					
					                                                      inv.setItem(0, 1, ChatColor.YELLOW + "Claim Value",
					                                                                  Material.GREEN_DYE, () -> {}, Stream.of(
											                                                      "&7This claim's estimated value is: &a$" +
											                                                      NumberFormatter.addCommas(c.getEstWorth()))
							                                                                  .map(x -> ChatColor.translateAlternateColorCodes(
									                                                                  '&', x)).collect(Collectors.toList())
					                                                      );
					                                                      inv.setItem(0, 2, InventoryHelper.getHead(
							                                                      Bukkit.getOfflinePlayer(UUID.fromString(c.owner)),
							                                                      "&eClaim Owner", List.of("&7Username: &e" +
							                                                                               Bukkit.getOfflinePlayer(
											                                                                               UUID.fromString(c.owner))
									                                                                               .getName())
					                                                      ), () -> {});
					                                                      inv.setItem(0, 3, ChatColor.YELLOW + "Rename claim",
					                                                                  Material.NAME_TAG, () -> {
								                                                      InventoryHelper.close(p);
								
								                                                      int id = 0;
								                                                      for (Claim tC : DatabasePlayer.from(p).getJsonPlayer()
										                                                      .getData().claims) {
									                                                      if (tC.location.hashCode() ==
									                                                          c.location.hashCode()) break;
									                                                      id++;
								                                                      }
								
								                                                      int finalId = id;
								                                                      ChatInput.createQuery("new name for claim",
								                                                                            response -> {
									                                                                                ...
									                                                                                20)
									                                                                            {
										                                                                            new MessageBuilder(
												                                                                            "Land").appendCaption(
														                                                                            "Name cannot be over 16 characters")
												                                                                            .send(p);
										                                                                            return;
									                                                                            }
									                                                                            DatabasePlayer.from(p)
									                                                                            ...
									                                                                            new MessageBuilder(
											                                                                            "Land").appendCaption(
													                                                                            "Land claim's name has been set to:")
											                                                                            .appendVariable(
													                                                                            response)
											                                                                            .send(p);
								                                                                            }, p
				                                                      ...
				                                                      if (ctx.equals("settings")) {
					                                                      inv.setMargin(1, 0);
					                                                      int                              index = 0;
					                                                      HashMap<String, BooleanRunnable> prefs = generatePreferenceTable(
							                                                      c);
					                                                      for (Map.Entry<String, BooleanRunnable> entry : prefs.entrySet()) {
						                                                      boolean value = entry.getValue().run(p, null);
						                                                      inv.setItem(0, index++, entry.getKey(),
						                                                                  value ? Material.GREEN_WOOL : Material.RED_WOOL,
						                                                                  () -> {
							                                                                  entry.getValue().run(p, !value);
						                                                                  }, Stream.of("&7Current value: " +
						                                                                               ( value ? "&a&lENABLED"
						                                                                                       : "&c&lDISABLED" ), "",
		...
		if (args.length >= 1 && args[0].equals("trusted")) {
			...
			if (args[1].equals("remove")) {
				...
				DatabasePlayer.from(p).getJsonPlayer().save();
				new MessageBuilder("Land").appendVariable(offlinePlayer.getName()).appendCaption("has been removed from the trust list")
						.send(p);
			}
