...
public class Claim {
	...
	public void transfer(UUID p) {
		this.destroy();
		DatabasePlayer.from(p).getJsonPlayer().getData().claims.add(new Claim(new Location(Bukkit.getWorld(location.world), location.startX, 0, location.startZ), new Location(Bukkit.getWorld(location.world), location.endX, 0, location.endZ), p));
