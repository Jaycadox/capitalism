...
public class BlueMapIntergration {
	...
	private void tick(Claim c, BlueMapAPI api) {
		...
		var shapeMarker = marker.createExtrudeMarker(c.location.hashCode() + "_shape", map,
		                                             new Vector3d(centerX, -64, centerZ),
		                                             Shape.createRect(new Vector2d(c.location.startX, c.location.startZ), new Vector2d(c.location.endX, c.location.endZ)),
		                                             -64,
		                                             319);
